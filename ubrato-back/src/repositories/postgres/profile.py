...
class ProfileRepository:
    ...
    async def set_brand_name(self, org_id: str, name: str) -> None:
        stmn = update(Organization).where(Organization.id == org_id).values(brand_name=name)
        await self.db.execute(stmn)

    async def get_customer(self, org_id: str) -> CustomerProfile:
        query = await self.db.execute(select(CustomerProfile).where(CustomerProfile.org_id == org_id))
    ...
    async def get_contractor_cv_by_id(self, cv_id: str) -> ContractorCV:
        query = await self.db.execute(select(ContractorCV).where(ContractorCV.id == cv_id))
    ...
    async def update_contractor_cv(self, cv: dict[str, Any], cv_id: str) -> None:
        query = await self.db.execute(select(ContractorCV).where(ContractorCV.id == cv_id))
    ...
    async def delete_contractor_cv(self, cv_id: str) -> None:
        await self.db.execute(delete(ContractorCV).where(ContractorCV.id == cv_id))
