...
from src.repositories.postgres.schemas import (
    ContractorProfile,
    CustomerProfile,
    Organization,
    User,
...
class UserRepository:
    ...
    async def get_by_email(self, email: str) -> models.User:
        query = await self.db.execute(select(User).where(User.email == email, User.deleted_at == None))
        user = query.scalar()
        if user is None:
    ...
    async def get_all_users(
        ...
        query = await self.db.execute(select(User).where(User.deleted_at == None))
    ...
    async def get_by_id(self, user_id: str) -> models.User:
        query = await self.db.execute(select(User).where(User.id == user_id, User.deleted_at == None))
    ...
    async def update_password(self, email: str, password: str) -> None:
        await self.db.execute(update(User).where(User.email == email).values(password=password))

        await self.db.commit()

    async def add_favorite_contratctor(self, user_id: str, contractor_id: str) -> None:
        self.db.add(UserFavoriteContractor(user_id=user_id, contractor_id=contractor_id))
        await self.db.commit()
    ...
    async def set_email_verified_status(self, user_id: str, verified: bool) -> None:
        await self.db.execute(update(User).where(User.id == user_id).values(email_verified=verified))
        await self.db.commit()
