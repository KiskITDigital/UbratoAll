...
from src.routers.v1.dependencies import authorized, get_user, localization
from src.schemas.change_password import ChangePasswordRequest
...
from src.schemas.sing_in import SignInRequest, SignInResponse
from src.schemas.success import SuccessResponse
from src.services import JWTService, NoticeService, OrganizationService, SessionService, UserService
...
async def signup_user(
    ...
    await user_service.ask_confirm_email(user_email=created_user.email, salt=access_token)

    return SignUpResponse(access_token=jwt_service.generate_jwt(created_user, created_org))
...
async def signin_user(
    ...
    return SignInResponse(access_token=jwt_service.generate_jwt(user=user, org=org))
...
async def ask_email_confirmation(
    ...
    await user_service.ask_confirm_email(user_email=user_email, salt=access_token)
    return SuccessResponse()
