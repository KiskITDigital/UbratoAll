...
from src.routers.v1.dependencies import is_admin, localization
...
from src.schemas.verify_status_set import VerifyStatusSet
from src.services import ManagerService, NoticeService, TenderService
...
async def user_verification_response(
    ...
    verf = await manager_service.get_verfication_request(verf_id=verification_id)
...
async def update_tender_verified_status(
    ...
    await manager_service.update_tender_verified_status(tender_id=tender_id, status=data.status)

    tender = await tender_service.get_by_id(tender_id=tender_id)

    notice_type = "tender_pass_verfication" if data.status else "tender_not_pass_verfication"
