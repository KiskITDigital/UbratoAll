...
from src.schemas.update_profile import (
    ContractorCVRequest,
...
async def update_my_customer_profile(
    ...
    await org_service.update_customer_info(org_id=user.org_id, description=data.description)
    await org_service.set_customer_locations(
        org_id=user.org_id,
        locations=[CustomerLocation(org_id=user.org_id, city_id=city_id) for city_id in data.locations],
    )
...
async def update_my_brand_contacts(
    ...
    await org_service.set_brand_contact_info(
        ...
        messengers=[(messenger.contact, messenger.info) for messenger in data.messengers],
    )
