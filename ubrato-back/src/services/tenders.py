...
from src.repositories.postgres import TagsRepository, TenderRepository
from src.repositories.postgres.schemas import Tender
from src.repositories.typesense.tender import TenderIndex
from src.schemas import models
from src.schemas.create_tender import CreateTenderRequest
from src.schemas.models import ObjectsGroupsWithTypes, ServicesGroupsWithTypes
...
class TenderService:
    ...
    async def create_tender(self, tender: CreateTenderRequest, user_id: str) -> models.Tender:
        ...
        return await self.tender_repository.get_tender_by_id(tender_id=created_tender.id)
    ...
    async def is_responded(self, tender_id: int, user_id: str) -> bool:
        return await self.tender_repository.is_responded(tender_id=tender_id, user_id=user_id)

    async def is_has_offer(self, tender_id: int, org_id: str) -> bool:
        return await self.tender_repository.is_has_offer(tender_id=tender_id, org_id=org_id)

    async def make_offer(self, contractor_id: str, tender_id: int, user_id: str) -> None:
        if not await self.has_permission_make_offer(user_id=user_id, tender_id=tender_id):
            raise ServiceException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=self.localization["errors"]["no_access"],
            )

        if await self.is_already_got_offer(contractor_id=contractor_id, tender_id=tender_id):
            raise ServiceException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def add_to_favorite(self, user_id: str, tender_id: int) -> None:
        await self.tender_repository.add_to_favorite(tender_id=tender_id, user_id=user_id)

    async def remove_from_favorite(self, user_id: str, tender_id: int) -> None:
        await self.tender_repository.remove_from_favorite(tender_id=tender_id, user_id=user_id)
